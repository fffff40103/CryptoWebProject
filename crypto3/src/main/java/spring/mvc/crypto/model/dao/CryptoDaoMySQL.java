...
public class CryptoDaoMySQL implements CryptoDao {
	...
	public int updateUserPassword(String username, String newPassword) {
		...
		int row = jdbcTemplate.update(sql2, newPassword, username);
		return row;
	}
	...
	public Optional<CryptoCurrency> findCryptoByCryptoId(Integer cNumber) {
		...
		try {
			CryptoCurrency crypto = jdbcTemplate.queryForObject(sql, new BeanPropertyRowMapper<>(CryptoCurrency.class),
					cNumber);
	...
	public int updateTopFiveCryptos(List<CryptoCurrency> cryptos) {
		...
		for (CryptoCurrency crypto : cryptos) {
			int count = jdbcTemplate.update(sql, crypto.getcName(), crypto.getPrice(), crypto.getRate(),
					crypto.getCap(), crypto.getcNumber());
			result += count;
		}
		return result;
	}
	...
	public boolean addTransactionDetail(TransactionDetail detail) {
		...
		return jdbcTemplate.update(sql, detail.getUserId(), detail.getcNumber(), detail.getQuantity(),
				detail.getPrice(), detail.getStatusId()) > 0;
	}
	...
	public List<TransactionDetail> findDetailByUserId(Integer userId) {
		...
		List<TransactionDetail> details = jdbcTemplate.query(sql, new BeanPropertyRowMapper<>(TransactionDetail.class),
				userId);
	...
	public Optional<StatusDetail> findStatusById(Integer statusId) {
		try {
			...
			StatusDetail statusDetail = jdbcTemplate.queryForObject(sql,
					new BeanPropertyRowMapper<>(StatusDetail.class), statusId);
